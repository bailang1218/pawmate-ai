...
from pawmate.core.safety.file_boundary import FileBoundaryError, assert_file_path_allowed, assert_text_file_readable
from pawmate.core.safety.resource_limits import MAX_TEXT_READ_BYTES, read_text_limited
from pawmate.tools.text.encoding import atomic_write_text, inspect_encoding

# 导入安全沙箱 - 支持两种导入模式（相对导入和绝对导入）
# 安全性：Sandbox 导入失败时必须 fail closed，禁止退化为无保护执行。
try:
    from pawmate.tools.security.sandbox import CommandValidator, Sandbox, SecurityViolation
...
def _safe_mcp_path(path: str, *, mode: str = "read") -> str:
    """在 MCP 工具中统一使用路径校验，避免绕过宿主安全策略。"""
    try:
        safe = CommandValidator.sanitize_path(path)
        assert_file_path_allowed(safe, mode=mode)
        return safe
    except (SecurityViolation, FileBoundaryError) as e:
...
def write_file(path: str, content: str, encoding: str = "utf-8") -> str:
    ...
    try:
        safe = _safe_mcp_path(path, mode="write")
    except SecurityViolation as e:
        return f"[security] {e}"
    target = Path(safe)
...
def delete_path(path: str) -> str:
    ...
    try:
        ...
        return f"[安全违规] {e}"
    return f"[security] delete_path requires host confirmation and is disabled on the MCP boundary: {safe}"
    if not p.exists():
        return "路径不存在，无需删除"
    try:
        if p.is_file():
