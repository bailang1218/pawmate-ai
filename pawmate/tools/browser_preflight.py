...
async def graceful_close_edge(timeout: float = 8.0) -> CloseResult:
    ...
    logger.warning(
        "[BrowserPreflight] Graceful Edge close timed out; escalating to taskkill /F"
    )
    subprocess.run(
    ...
    while time.monotonic() < deadline:
        if not _has_browser_proc():
            return CloseResult(closed=True, forced=True)
        await asyncio.sleep(0.3)
    return CloseResult(closed=False, forced=True)
