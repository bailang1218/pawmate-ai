...
from pawmate.tools.registry import ToolDef, ToolRegistry, APPROVAL_CONFIRM, APPROVAL_NOTIFY
from pawmate.core.path_security import PathSecurityError
from pawmate.core.security_service import SecurityService
from pawmate.tools.sandbox import Sandbox, SecurityViolation
from pawmate.tools.vision_stub import capture_screenshot, ocr_image, vision_analyze
from pawmate.tools.download_tool import register_download_tools
from pawmate.tools.browser_facade import register_browser_facade_tools
from pawmate.core.browser_diagnostics import log_browser_diagnostics
...
async def run_shell_command(command: str, cwd: str = "", timeout: int = 20) -> str:
    from pawmate.tools.text_encoding import validate_shell_payload
...
from pawmate.tools.text_encoding import inspect_encoding, atomic_write_text
...
def register_builtin_tools(
    ...
    if os.getenv("PAWMATE_ENABLE_LEGACY_CHATGPT_BRIDGE", "").strip().lower() in {"1", "true", "yes", "on"}:
        from pawmate.tools.chatgpt_bridge import register_chatgpt_bridge_tools
