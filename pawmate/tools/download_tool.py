...
from pawmate.tools.registry import ToolDef, ToolRegistry
...
async def download_file(
    ...
    try:
        ...
        return await _download_file_impl(url, dest_path, file_path, timeout)

    except Exception as e:
        return {"ok": False, "operation": "download_file", "error_type": "prepare_error", "message": str(e), "url": url}


def _prepare_download_target(url: str, destination_folder: str, filename: str) -> tuple[Path, Path, str]:
...
async def download_with_metadata(
    ...
    try:
        ...
        # 执行下载
        result = await _download_file_impl(url, dl_dest_path, dl_file_path, timeout)
