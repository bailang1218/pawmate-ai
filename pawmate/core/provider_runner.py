...
from pawmate.core.llm_factory import DEFAULT_LLM_CONFIG, get_llm_client
from pawmate.core.llm_provider import LLMProvider
from pawmate.core.llm_router import RouteChoice
...
class ProviderRunner:
    ...
    async def collect_stream_events(
        ...
        *,
        ...
        for attempt_idx in range(len(self._fallback_providers)):
            provider = self._fallback_providers[attempt_idx]
            if attempt_idx > 0:
                print(f"[FALLBACK] switching to {provider} after {' -> '.join(provider_tried)}")
                event_bus.publish(TextDeltaEvent(f"\n[切换到 {provider}] ", turn_id=turn_id))
                try:
                    self._llm = self._new_client(provider, self._fallback_models.get(provider))
                    self._current_provider = provider
                    self._current_model = (
                        getattr(self._llm, "_model", None)
                        or getattr(self._llm, "model", self._fallback_models.get(provider))
                    )
                except Exception as exc:
                    print(f"[FALLBACK] {provider} 瀹㈡埛绔垱寤哄け璐? {exc}")
            ...
            for stream_attempt in range(self._SAME_PROVIDER_RETRIES):
                try:
                    ...
                    print(f"[FALLBACK] {provider} 娴佸紡璋冪敤澶辫触: {exc}")
                    stream_error = exc
                    if current_text or not self._should_retry_same_provider(exc, stream_attempt):
                        break
                    ...
                    if attempt_idx > 0:
                        print(f"[FALLBACK] {provider} 闈炵綉缁滈敊璇紙涓嶈Е鍙?fallback锛? {exc}")
                    raise
    ...
    async def _recreate_current_client(self, provider: str) -> None:
        ...
        self._llm = self._new_client(provider, model)
        self._current_provider = provider
        self._current_model = getattr(self._llm, "_model", None) or getattr(self._llm, "model", model)
        ...
        return [
            *messages,
            {"role": "assistant", "content": partial},
            {
                "role": "user",
                "content": (
                    "The previous model provider failed after the assistant text above. "
