...
class ChatService(QObject):
    ...
    external_text_delta = Signal(str, int, str)
    external_finished = Signal(str, int)
    external_stream_end = Signal(str, int)
    external_error = Signal(str, int, str)
    external_tool_event = Signal(str, dict)
    external_cancelled = Signal(str, int)
    ...
    def __init__(
        ...
        # AgentEngine still owns shared history, routing, approval and checkpoint
        # state. Admit only one turn until those are isolated per turn.
    ...
    def submit_user_message(self, text: str) -> int:
        """Submit a regular Web chat turn and return its turn id."""
        return self._submit_message(text, source="web")

    def submit_external_message(self, text: str, *, source: str) -> int:
        """Submit a turn owned by another UI surface, such as the Agent CLI."""
        normalized_source = str(source or "").strip().lower()
        if not normalized_source or normalized_source == "web":
            raise ValueError("External message source must be a non-web identifier")
        return self._submit_message(text, source=normalized_source)

    def _submit_message(self, text: str, *, source: str) -> int:
        message = text.strip()
        if not message:
        ...
        if len(self._active_turns) >= self._max_concurrent_tasks:
            ...
            error = "Too many concurrent tasks; wait for one to finish."
            if source == "web":
                self.error.emit(error)
            else:
                self.external_error.emit(source, 0, error)
            return 0

        self._logger.info(
            "[ChatService] %s submit source=%s: %.80s",
            "mock" if self._mock_mode else "real",
            source,
            message,
        )
        turn_id = self._begin_turn(message, source=source)
        if self._mock_mode:
    ...
    def cancel_turn(self, turn_id: int) -> bool:
        ...
        self._logger.info("[ChatService] cancel turn=%s", turn_id)
        ...
        source = self.get_turn_source(turn_id)
        if source == "web":
            self.cancelled_turn.emit(turn_id)
            self.cancelled.emit()
        else:
            self.external_cancelled.emit(source, turn_id)
        self._finalize_turn("cancelled", turn_id)
        return True

    def get_turn_source(self, turn_id: int) -> str:
        state = self._active_turns.get(int(turn_id or 0))
        if state is not None:
            return str(state.get("source") or "web")
        record = self._task_records.get(int(turn_id or 0), {})
        return str(record.get("source") or "web")

    def is_turn_active(self, turn_id: int) -> bool:
        return int(turn_id or 0) in self._active_turns
    ...
    def _handle_real(self, message: str, turn_id: int) -> None:
        ...
        if self._queue_until_worker_ready and not self._is_worker_ready(self._worker):
            ...
            self._update_task(turn_id, status="queued", detail="waiting for backend")
            self._dispatch_when_worker_ready(message, turn_id, elapsed_ms=0)
            return
        self._mark_turn_dispatched(turn_id)
    ...
    def _on_text_delta(self, text: str, turn_id: int = 0) -> None:
        ...
        if text:
            ...
            if turn_id == self._current_turn_id:
                self.status_changed.emit("answering...")
    ...
    def _begin_turn(self, message: str = "", *, source: str = "web") -> int:
        ...
        logging.getLogger("pawmate.operation").info(
            "[Turn] started id=%s title=%s",
            turn_id,
            redact_text(message or f"Task {turn_id}")[:200],
        )
        return turn_id
    ...
    def _ensure_stream(self, turn_id: int = 0) -> None:
        ...
        if not state.get("stream_active"):
            ...
            if source == "web":
                ...
                self.external_text_delta.emit(source, turn_id, "")

    def _finalize_stream(self, turn_id: int = 0) -> None:
        turn_id = turn_id or self._current_turn_id
        self._stream_trace.flush(turn_id, reason="finalize")
        state = self._active_turns.get(turn_id)
        ...
        if source == "web":
            ...
            self.external_stream_end.emit(source, turn_id)
        if turn_id == self._current_turn_id:
            self._stream_active = False

    def _mark_turn_dispatched(self, turn_id: int) -> None:
        if self._is_stale_turn(turn_id):
            return
        if turn_id == self._current_turn_id:
            self.status_changed.emit("thinking...")
        self._update_task(turn_id, status="thinking", detail="model request")
    ...
    def _finalize_turn(self, reason: str, turn_id: int = 0) -> None:
        ...
        if source == "web":
            ...
            self.external_finished.emit(source, turn_id)
        self._sync_legacy_state()
        ...
        logging.getLogger("pawmate.operation").info(
            "[Turn] finished id=%s reason=%s active=%d",
            turn_id,
            reason,
    ...
    def _emit_text_delta(self, text: str, turn_id: int) -> None:
        ...
        self._stream_trace.record(
            turn_id,
            component="chat_service.emit",
            turn_id=turn_id,
            seq=count,
            text=text,
            turn_receivers=turn_receivers,
            seq_receivers=seq_receivers,
            legacy_receivers=qt_receiver_count(self, self.text_delta, "text_delta(QString)"),
        )
        source = self.get_turn_source(turn_id)
        if source == "web":
            if seq_receivers:
            ...
            self.external_text_delta.emit(source, turn_id, text)
    ...
    def _emit_error(self, message: str, turn_id: int) -> None:
        ...
        if source == "web":
            ...
            self.external_error.emit(source, turn_id, message)
    ...
    def _emit_tool_event(self, payload: dict[str, Any], turn_id: int) -> None:
        ...
        if source == "web":
            ...
            self.external_tool_event.emit(source, data)
