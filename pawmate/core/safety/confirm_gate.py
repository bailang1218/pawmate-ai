...
from pawmate.core.safety.redaction import redact_for_json, redact_sensitive_data, redact_text
from pawmate.core.tools.tool_policy import can_auto_approve_request
...
class ConfirmGate:
    ...
    def resolve(self, allowed: bool) -> None:
        ...
        """
        from pawmate.tools.core.registry import ApprovalDecision
    ...
    async def request_confirmation(self, name: str, input_data: dict) -> bool:
        ...
        async with self._request_lock:
            ...
            if self._auto_approve and self._can_auto_approve(name, input_data):
                self._registry._last_approval = ApprovalDecision(
                    allowed=True,
    ...
    def _can_auto_approve(self, name: str, input_data: dict | None = None) -> bool:
        get_tool = getattr(self._registry, "get_tool", None)
        tool_def = get_tool(name) if callable(get_tool) else None
        if tool_def is None:
            return False
        return can_auto_approve_request(tool_def, input_data or {})
