...
def normalize_security_config(sec: dict[str, Any] | None) -> dict[str, Any]:
    ...
    allowed_roots = raw.get("allowed_roots") or raw.get("allowed_path")
    if not allowed_roots or allowed_roots == ["~"]:
        allowed_roots = [str(get_project_root())]
    ...
    return {
        ...
        "allow_browser_process_launch": raw.get("allow_browser_process_launch") is True,
    }
